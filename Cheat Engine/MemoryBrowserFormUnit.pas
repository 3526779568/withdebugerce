...
resourcestring
  ...
  rsCheatEngineSingleLingeAssembler = 'DDY Engine single-line assembler';
  ...
  rsPleaseTargetAnotherProcess = 'Start another version of DDY Engine and attach to that instead';
...
procedure TMemoryBrowser.N16Click(Sender: TObject);
begin
...
begin
  ...
  try
    if reg.OpenKey('\Software\DDY Engine\Disassemblerview\',true) then
    begin
    ...
    if reg.OpenKey('\Software\DDY Engine\Disassemblerview\Font',true) then
      SaveFontToRegistry(disassemblerview.font, reg);

    if reg.OpenKey('\Software\DDY Engine\Hexview',true) then
    begin
      reg.writeInteger('spaceBetweenLines', hexview.spaceBetweenLines);
      reg.WriteBool('showStatusBar', hexview.statusbar.Visible);
    end;

    if reg.OpenKey('\Software\DDY Engine\Hexview\Font',true) then
...
begin
  ...
  try
    ...
    if reg.OpenKey('\Software\DDY Engine\Disassemblerview\Font',false) then
    begin
      LoadFontFromRegistry(f, reg);
      disassemblerview.font:=f;
    end;

    if reg.OpenKey('\Software\DDY Engine\Disassemblerview\',false) then
    begin
    ...
    if reg.OpenKey('\Software\DDY Engine\Hexview',false) then
    begin
    ...
    if reg.OpenKey('\Software\DDY Engine\Hexview\Font',false) then
    begin
      LoadFontFromRegistry(f, reg);
      hexview.hexfont:=f;
    end;
...
begin
  ...
  begin
    ...
    begin
      beep; //Best sound effect DDY Engine has
      exit;
    end;
  end;
...
{will save a DDY Engine memory region file .CEM}
begin
