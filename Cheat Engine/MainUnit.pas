...
resourcestring
  ...
  rsThankYouForTryingOutCheatEngineBecauseItHasExpired =
    'Thank you for trying out DDY Engine. Because it has expired DDY Engine will now close. Is that ok with you?';
  ...
  strClickToGoHome = 'Click here to go to the DDY Engine homepage';
  ...
  rsDoYouWantToGoToTheCheatEngineWebsite =
    'Do you want to go to the DDY Engine website?';
  ...
  rsBringsCheatEngineToFront = 'brings DDY Engine to front';
  ...
  strfuture = 'Wow,I never imagined people would use DDY Engine up to today';
  rsLicenseExpired =
    'Your license to use DDY Engine has expired. You can buy a license to use DDY Engine for 1 month for $200, 6 months for only $1000 and for 1 year for ' + 'only $1800. If you don''t renew your license DDY Engine will be severely limited in it''s abilities. (e.g: Next scan has been disabled)';
  rsEXPIRED = 'EXPIRED';
  strdontbother =
    'Don''t even bother. DDY Engine uses the main thread to receive messages when the scan is done, freeze it and CE will crash!';
  ...
  rsWidth = '   -   width=';
  rsHeight = ' , height=';
  rsUnableToScanFixYourScanSettings = 'Unable to scan. Fix your scan settings and restart DDY Engine';
...
begin
  ...
  try
    if reg.OpenKey('\Software\DDY Engine\CustomTypes\', False) then
    begin
      CustomTypes := TStringList.Create;
      reg.GetKeyNames(CustomTypes);
      for i := 0 to CustomTypes.Count - 1 do
      begin
        if reg.OpenKey('\Software\DDY Engine\CustomTypes\' + CustomTypes[i], False) then
        begin
          try
            islua := False;
...
begin
  ...
  begin
    ...
    begin
      reg := tregistry.Create;
      reg.DeleteKey('\Software\DDY Engine\CustomTypes\' + ct.Name);
      ct.remove;
      RefreshCustomTypes;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        //delete the old one
        reg := Tregistry.Create;
        reg.DeleteKey('\Software\DDY Engine\CustomTypes\' + oldname);
        freeandnil(reg);
      end;
    end;
    ...
    reg := Tregistry.Create;
    if Reg.OpenKey('\Software\DDY Engine\CustomTypes\' + ct.Name, True) then
    begin
...
begin
  if messagedlg(rsDoYouWantToGoToTheCheatEngineWebsite, mtConfirmation,
    [mbYes, mbNo], 0) = mrYes then
    ShellExecute(0, PChar('open'), PChar('http://www.baidu.org/?referredby=CE67'),
...
begin
  ...
  try
    ...
    if not Reg.OpenKey('\Software\DDY Engine', False) then
    begin
      if Reg.OpenKey('\Software\DDY Engine', True) then
      begin
