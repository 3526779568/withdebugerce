...
begin
  ...
  begin
    ...
    if Reg.OpenKey('\Software\DDY Engine\'+ClassName, true) then
    begin
    ...
    if Reg.OpenKey('\Software\DDY Engine\PSNNodeList', false) then
    begin
    ...
    begin
      if iplist[i].host<>'' then
      begin
        if Reg.OpenKey('\Software\DDY Engine\PSNNodeList\'+iplist[i].host+':'+iplist[i].port,true) then
        begin
...
begin
  ...
  if Reg.OpenKey('\Software\DDY Engine\'+ClassName, false) then
  begin
  ...
  begin
    ...
    for i:=0 to list.count-1 do
    begin
      if reg.OpenKey('\Software\DDY Engine\PSNNodeList\'+list[i], false) then
      begin
