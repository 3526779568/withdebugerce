...
resourcestring
  ...
  rsCEAFilter = 'DDY Engine Assembly (*.CEA)|*.CEA|All Files ( *.* )|*.*';
  rsCEGAFilter = 'DDY Engine GNU Assembly (*.CEGA)|*.CEGA|All Files ( *.* )|*.*';
...
{  assemblescreen.SelStart:=0;
  ...
  try
    if reg.OpenKey('\Software\DDY Engine\Auto Assembler\',false) then
    begin
  ...
  begin
    try
      ...
      begin
        ...
        reg:=tregistry.create;
        try
          if reg.OpenKey('\Software\DDY Engine\Auto Assembler\',true) then
          begin
...
begin
  ...
  // same as menu option "DDY Engine framework code", make sure we
